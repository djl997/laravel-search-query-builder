...
class LaravelSearchQueryBuilderServiceProvider extends ServiceProvider
{
    ...
    {
        Builder::macro('search', function(array $fields, string $queryString) {
            $strings = Str::of(Str::squish($queryString))->explode(',');
            
            // Group in where, just to be sure
            $this->where(function(Builder $query) use ($fields, $strings) {
                foreach($strings as $string) {
                    foreach($fields as $field) {
                        // Add foreach search term and column a `orWhere` query
